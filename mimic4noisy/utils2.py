...
def select_memorization_and_forgetting_per_epoch(pred_all, labels, clean_labels, epoch, lambda_coeff=1.0):
    ...
    # Adding Co-Occurrence Metric
    # co_matrix = compute_cooccurrence_matrix(labels)


    # # Co-occurrence adjustment (batch-wise matrix multiplication)
    # co_occurrence_effect = 0.3 * torch.matmul(selection_metric, co_matrix)
    # selection_metric += co_occurrence_effect
...
def fit_local_gmm_and_correct_labels(args, labels, pred_labels, clean_mask, negative_pair_mask, pos_selection_metric, neg_selection_metric):
    ...
    for class_idx in range(num_classes):
        ...
        if len(neg_class_selection_metric) > 1:
            ...
            # For noisy label, use prediction
            # labels[neg_noisy_indices, class_idx] = (0.9 * pred_labels[neg_noisy_indices, class_idx].to(labels.dtype) +
            #     (0.1) * labels[neg_noisy_indices, class_idx]
            # ).to(labels.dtype)  
            ...
            # print(f"Negative Clean Indices: {neg_clean_indices.sum().item()}")
            # print(f"Negative Noisy Indices: {neg_noisy_indices.sum().item()}")
...
# LL-R
def calculate_R_t(loss_all, t, delta_rel):
    # Flatten loss values up to epoch t
    loss_values = loss_all[:, :, :t].reshape(-1)
    
    # Sort in descending order
    sorted_losses, _ = torch.sort(loss_values, descending=True)
    
    # Calculate the threshold index
    threshold_index = int((t - 1) * delta_rel * len(sorted_losses) / 100)
    
    # Get the R(t) value
    R_t = sorted_losses[threshold_index]
...
def loss_adaptation(loss_all, ranks_all, mems_all, args, y_pred, y_noisy, clean_mask, epoch, x_ids):
    """
    Adapts the loss based on LL-R, LL-Ct methods while ensuring clean samples are not rejected.

    Args:
        loss_all (torch.Tensor): Historical loss values across epochs.
        ranks_all (torch.Tensor): Sample ranking across epochs.
        mems_all (torch.Tensor): Memory tracking losses across epochs.
        args (argparse.Namespace): Training arguments including delta_rel.
        y_pred (torch.Tensor): Model predictions.
        y_noisy (torch.Tensor): Noisy labels.
        clean_mask (torch.Tensor): Mask indicating clean samples (1 for clean, 0 for noisy).
        epoch (int): Current training epoch.
        x_ids (torch.Tensor): Indices of the current batch.

    Returns:
        torch.Tensor: Adapted loss.
    """
    if epoch < 3:
        raise ValueError("Epoch should be at least 3 to compute historical loss.")

    # Compute historical means
    mean_loss = torch.sum(loss_all[x_ids, :, :epoch], dim=-1)
    rank_loss = torch.sum(ranks_all[x_ids, :, :epoch], dim=-1)
    mems_loss = torch.sum(mems_all[x_ids, :, :epoch], dim=-1)
    
    
    # Define positive and negative metrics
    neg_metric = normalize(mean_loss) # Metric for negative samples (high memory usage indicates potential noise)
    pos_metric = normalize(mems_loss)  # Combined metric for positive samples

    delta_rel = args.delta_rel if hasattr(args, 'delta_rel') else 0.1
    num_samples = mean_loss.numel()
    k = max(1, int(num_samples * (epoch - 1) * delta_rel / 100))

    # Compute thresholds for positive and negative metrics
    neg_threshold = torch.topk(neg_metric.flatten(), k=k).values[-1]
    pos_threshold = torch.topk(pos_metric.flatten(), k=k).values[-1]

    # Generate rejection masks separately for positive and negative samples
    positive_pair_mask = (y_noisy == 1)
    negative_pair_mask = (y_noisy == 0)

    reject_mask_pos = (pos_metric > pos_threshold)  & positive_pair_mask
    reject_mask_neg = (neg_metric > neg_threshold)  & negative_pair_mask

    
    
    # Initialize loss weights (1 for all samples, 0 for rejected ones)
    loss_weight = torch.ones_like(y_pred)
    loss_weight[reject_mask_pos] = 0  # Suppress positive samples that exceed threshold
    loss_weight[reject_mask_neg] = 0  # Suppress negative samples that exceed threshold


    # Compute final loss with the adapted weights
    criterion = torch.nn.BCELoss(reduction='none')
    loss = criterion(y_pred, y_noisy.float()) * loss_weight
    loss = loss.mean()
