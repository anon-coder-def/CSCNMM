...
from utils import Discretizer, Normalizer, my_metrics, is_ascending, robust_metrics, MultiLabelRankingLoss
from dataset.dataloader import get_multimodal_datasets
from mymodel.model import FlexCare
from mimic4noisy.utils2 import *
...
def my_collate(batch):
    ...
    # We don't have image data for now... use all torch.zeros for now, make mask vector all 0
    cxr = torch.stack([torch.zeros(3, 224, 224) for item in batch])
    mask_cxr = np.array([0 for item in batch])
    ...
    # Label
    noisy_label = np.array([item[4] for item in batch]).reshape(len(batch),-1)
    ...
    x_ids = [item[8] for item in batch]
    
    
    return [ehr, ehr_length, mask_ehr, cxr, mask_cxr, note, mask_note, label, noisy_label, task_index, x_ids]
...
def main(seed):
    ...
    for t in range(len(args.task)):
        ...
        train_dl = DataLoader(train_ds, args.batch_size, shuffle=False, collate_fn=my_collate, pin_memory=True, num_workers=num_workers, drop_last=False)
        test_dl = DataLoader(test_ds, args.batch_size, shuffle=True, collate_fn=my_collate, pin_memory=True, num_workers=num_workers, drop_last=False)
    ...
    each_criterion_ce = torch.nn.CrossEntropyLoss(reduction='none')
    ...
    for epoch in tqdm(range(1, args.epochs+1)):
        ...
        for t in range(len(mutli_train_dl)):
            ...
            with tqdm(mutli_train_dl[t], position=0, ncols=150, colour='#666666') as tqdm_range:
                for i, data in enumerate(tqdm_range):
                    optimizer.zero_grad()
                    ehr, ehr_length, mask_ehr, cxr, mask_cxr, note, mask_note, label, noisy_label, task_index, x_ids = data
                    ...
                    if task_now in ['length-of-stay','drg']:
                        ...
                        criterion_now_each = each_criterion_ce
                        
                        y_noisy = y_noisy.long().view(-1)
                    else:
                    # Binary Class (Multi-label: have multiple binary class)
                    ...
                    if task_now in ['phenotyping','diagnosis']:
                        ...
                        # Compute Diversity
                        diversity = calculate_diversity(y_pred.data.detach().clone().cpu().numpy(), y_noisy.data.detach().clone().cpu().numpy())
                   ...
                   # Correction Starts
                    if epoch > args.warmup and args.corr:
                        ...
                        loss = (total_loss + ortho_loss * 0.5 + moe_loss) * task_weight[task_now]
        ...
        if epoch > args.warmup-1:
            ...
            corr_loss, corr_rank_loss, clean_mask, negative_pair_mask, corr_labels = fit_gaussian_model_loss_corr_latest(
                ...
                C, 
                torch.tensor(divers_all[t][:, :, epoch-1], device=device),
                criterion_now_each, 
        ...
        stats_over_epochs = visualization(epoch, loss_all[t], mems_all[t], ranks_all[t], pred_all[t], divers_all[t], true_all[t], noisy_all[t], stats_over_epochs, save_dir)
    ...
    plot_statistics(stats_over_epochs, save_dir) 
